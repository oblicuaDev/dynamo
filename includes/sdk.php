...
class DynamoSDK {
  ...
  // Declare constants for base URL and API key
  public $domain = 'https://dynamo.mottif.tv/home/bo39bipxontb/public_html/dynamo.mottif.tv/drpl/api/v1/';
  public $infoGnrl = array();
  public $language = "";
  public $globalURL = "https://dynamo.mottif.tv";
  public $production = true;
  // Constructor function to initialize the HTTP client
...
// }
  ...
  function gNews($id = "", $filter = false){
    if($id != ""){
      $result = $this->query("noticias/".$id);
      return $result[0];
    }else{
      if($filter){
        $result = $this->query("noticias/all", "", ["field_on_slider_value=1"]);
      }else{
        $result = $this->query("noticias/all");
      }
      return $result;
    }
  }
  function gPrensa($id = "", $filter = false){
    if($id != ""){
      $result = $this->query("prensa/".$id);
      return $result[0];
    }else{
      if($filter){
        $result = $this->query("prensa/all", "", ["field_on_slider_value=1"]);
      }else{
        $result = $this->query("prensa/all");
      }
      return $result;
    }
  }
  function getTaxs($id = ""){
    $result = $this->query("tax/".$id."/all");
    return $result[0]->field_logo;
  }
  function getTaxonomy($id = "all", $voc = "all"){
    $result = $this->query("tax/".$id."/".$voc);
    return $result;
  }
  function gMovies($id = "", $propia = false){
    if($id != ""){
      $result = $this->query("movies/".$id);
      return $result[0];
    }else{
      if($propia){
        $result = $this->query("movies/all", "", ["field_produccion_propia_value=0"]);
      }else{
        $result = $this->query("movies/all", "", ["field_produccion_propia_value=1"]);
      }
      return $result;
    }
  }
  function gSeries($id = ""){
    if($id != ""){
      $result = $this->query("series/".$id);
      return $result[0];
    }else{
      $result = $this->query("series/all");
      return $result;
    }
  }
  function gTemporadas($id = "", $propia=false){
    if($id != ""){
      $result = $this->query("temporadas/".$id);
      return $result[0];
    }else{
      if($propia){
        $result = $this->query("temporadas/all", "", ["field_produccion_propia_value=0"]);
      }else{
        $result = $this->query("temporadas/all", "", ["field_produccion_propia_value=1"]);
      }
      return $result;
    }
  }
  function getBanners($id = ""){
    if($id != ""){
      $result = $this->query("banners/".$id);
      return $result[0];
    }else{
      $result = $this->query("banners/all");
      return $result;
    }
  }
  function gPersons($id = ""){
   
    if($id != ""){
      $result = $this->query("team/".$id);
      return $result[0];
    }else{
      $socios = array();
      $colaborador1 = array();
      $result = $this->query("team/all");
      for ($i=0; $i < count($result); $i++) { 
        switch ($result[$i]->field_posicion_list) {
          case '3':
            array_push($socios, $result[$i]);
            break;
          case '4':
            array_push($colaborador1, $result[$i]);
            break;
        }
      }
      $all["socios"] = $socios;
      $all["colaborador1"] = $colaborador1;
      return $all;
    }
  }
  function gBanners(){
      $result = $this->query("banners_cuadrados");
      return $result;
  }
  function gVacantes(){
      $result = $this->query("vacantes");
      return $result;
  }
  function gMovieBanners($propia = false){
    if($propia){
      $result = $this->query("banners-movies", "", ["field_produccion_propia_value=1"]);
    }else{
      $result = $this->query("banners-movies", "", ["field_produccion_propia_value=0"]);
    }
      return $result;
  }
  function gSeriesBanners($propia = false){
    if($propia){

      $result = $this->query("banners-temporadas", "", ["field_produccion_propia_value=1"]);
    }else{
      $result = $this->query("banners-temporadas", "", ["field_produccion_propia_value=0"]);
    }
      return $result;
  }
  function gFaq(){
      $result = $this->query("faq");
      return $result;
  }
  function gSearch($word){
      $result = $this->query("search/". $word);
      return $result;
  }

  function insertForm($content_for,$type_for,$parent_for){
    $date = new DateTime('now', new DateTimeZone('America/Bogota'));
    $db = new dbase();
    $insert = $db->insert("forms",array("content_for","type_for","parent_for"),array($content_for,$type_for,$parent_for));
    return $insert;
  }
